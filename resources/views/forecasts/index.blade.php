    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            setTimeout(() => {
                ...
                const realHeader = document.getElementById('RealPageContent');
                const realBody = document.getElementById('MainContent');

                if(pageSkeleton) pageSkeleton.remove();

                if(realHeader) {
                    realHeader.classList.remove('hidden');
                    setTimeout(() => realHeader.classList.remove('opacity-0'), 10);
                }

                if(realBody) {
                    realBody.classList.remove('hidden');
                    setTimeout(() => realBody.classList.remove('opacity-0'), 10);
                }
            }, 500);
        });
