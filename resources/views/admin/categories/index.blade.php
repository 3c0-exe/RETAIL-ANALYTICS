<x-app-layout>
    <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <!-- Categories List View (Mobile & Desktop) -->
                    ...
                    <div class="flex items-center gap-3 p-4">
                        ...
                        <div class="flex-shrink-0">
                            @if($category->is_active)
                                <span class="px-2.5 py-1 text-xs font-medium text-green-700 bg-green-100 rounded-full dark:bg-green-900/20 dark:text-green-400">
                                    Active
                                </span>
                            @else
                                <span class="px-2.5 py-1 text-xs font-medium text-gray-700 bg-gray-100 rounded-full dark:bg-gray-800 dark:text-gray-400">
                                    Inactive
                                </span>
                            @endif
                        </div>
            ...
            @forelse($categories as $category)
                <div class="bg-white dark:bg-[#171717] border border-gray-200 dark:border-gray-800 rounded-lg p-4 hover:shadow-lg transition-shadow category-card"
                    ...
                    <div class="flex items-start justify-between mb-3">
                        ...
                        @if($category->is_active)
                            <span class="px-2 py-1 text-xs font-medium text-green-700 bg-green-100 rounded-full dark:bg-green-900/20 dark:text-green-400 flex-shrink-0 ml-2">
                                Active
                            </span>
                        @else
                            <span class="px-2 py-1 text-xs font-medium text-gray-700 bg-gray-100 rounded-full dark:bg-gray-800 dark:text-gray-400 flex-shrink-0 ml-2">
                                Inactive
                            </span>
                        @endif
                    </div>
        ...
        <!-- Categories Table View (Desktop Only) -->
    ...
    <script>
        ...
        function searchCategories() {
            ...
            const listCards = document.querySelectorAll('.category-list-card');
            ...
            const gridCards = document.querySelectorAll('.category-card');
        ...
        function updateResultCount() {
            ...
            if (currentView === 'list') {
                visibleCount = Array.from(document.querySelectorAll('.category-list-card'))
                    .filter(card => card.style.display !== 'none').length;
            } else if (currentView === 'grid') {
                visibleCount = Array.from(document.querySelectorAll('.category-card'))
                    .filter(card => card.style.display !== 'none').length;
            } else if (currentView === 'table') {
                visibleCount = Array.from(document.querySelectorAll('.category-row'))
                    .filter(row => row.style.display !== 'none').length;
            }

            const countText = `${visibleCount} ${visibleCount === 1 ? 'category' : 'categories'}`;
            const resultCount = document.getElementById('resultCount');
            if (resultCount) resultCount.textContent = countText;
        }

        document.addEventListener('DOMContentLoaded', function() {
            // Get saved view preference, default to 'list' for better mobile experience
            const savedView = localStorage.getItem('categoryView') || 'list';

            // On mobile, force list view for better UX
            ...
            window.addEventListener('resize', function() {
                ...
                // If switching to mobile and table view is active, switch to list
