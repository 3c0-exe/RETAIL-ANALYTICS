<x-app-layout>
    <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-4 sm:py-6">
            ...
            <ol class="inline-flex items-center space-x-1 md:space-x-2">
                ...
                <li>
                    <div class="flex items-center">
                        <svg class="w-3 h-3 mx-1 text-gray-400 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 6 10">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m1 9 4-4-4-4"/>
                        </svg>
                        <a href="{{ route('admin.imports.index') }}" class="text-xs sm:text-sm text-gray-700 hover:text-primary-600 dark:text-gray-400 dark:hover:text-primary-400 transition-colors">
                            Imports
                        </a>
                    </div>
                </li>
                <li aria-current="page">
                    <div class="flex items-center">
                        ...
                        <span class="text-xs sm:text-sm font-medium text-gray-500 dark:text-gray-400">Details</span>
                    </div>
                </li>
            </ol>
        ...
        <div class="mb-6 sm:mb-8">
            <h1 class="text-xl sm:text-2xl lg:text-3xl font-bold text-gray-900 dark:text-gray-100">Import Details</h1>
            <p class="mt-1 text-xs sm:text-sm text-gray-600 dark:text-gray-400">View and manage import information</p>
        </div>

        <div class="space-y-4 sm:space-y-6">
            <div id="DetailSkeleton" class="animate-pulse space-y-4 sm:space-y-6">
                <div class="bg-white dark:bg-[#171717] border border-gray-200 dark:border-gray-800 rounded-lg p-4 sm:p-6">
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
                        @for($i=0; $i<4; $i++)
                        <div class="space-y-2">
                            <div class="h-3 sm:h-4 bg-gray-200 rounded dark:bg-gray-700 w-20 sm:w-24"></div>
                            <div class="h-5 sm:h-6 bg-gray-200 rounded dark:bg-gray-700 w-28 sm:w-32"></div>
                        </div>
                        @endfor
                    </div>
                    <div class="mt-6 h-9 sm:h-10 bg-gray-200 rounded dark:bg-gray-700 w-36 sm:w-40"></div>
                </div>
                <div class="bg-white dark:bg-[#171717] border border-gray-200 dark:border-gray-800 rounded-lg p-4 sm:p-6 space-y-3 sm:space-y-4">
                    <div class="h-5 sm:h-6 bg-gray-200 rounded dark:bg-gray-700 w-40 sm:w-48 mb-4"></div>
                    <div class="space-y-2 sm:space-y-3">
                        @for($j=0; $j<5; $j++)
                        <div class="h-7 sm:h-8 bg-gray-200 rounded dark:bg-gray-700 w-full"></div>
                        @endfor
                    </div>
                </div>
            </div>

            <div id="RealDetailContent" class="hidden opacity-0 transition-opacity duration-500 space-y-4 sm:space-y-6">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
                            <div class="min-w-0">
                                <div class="flex items-center gap-2 mb-2">
                                    ...
                                    <h4 class="text-xs sm:text-sm font-medium text-gray-500 dark:text-gray-400">File Name</h4>
                                </div>
                                <p class="text-sm sm:text-base font-semibold text-gray-900 dark:text-gray-100 truncate" title="{{ $import->file_name }}">
                                    {{ $import->file_name }}
                                </p>
                            </div>
                            ...
                            <div class="min-w-0">
                                <div class="flex items-center gap-2 mb-2">
                                    ...
                                    <h4 class="text-xs sm:text-sm font-medium text-gray-500 dark:text-gray-400">Branch</h4>
                                </div>
                                <p class="text-sm sm:text-base font-semibold text-gray-900 dark:text-gray-100 truncate" title="{{ $import->branch->name ?? 'N/A' }}">
                            ...
                            <div>
                                <div class="flex items-center gap-2 mb-2">
                                    ...
                                    <h4 class="text-xs sm:text-sm font-medium text-gray-500 dark:text-gray-400">Status</h4>
                                </div>
                                @php
                                    $statusColors = [
                            ...
                            <div>
                                <div class="flex items-center gap-2 mb-2">
                                    ...
                                    <h4 class="text-xs sm:text-sm font-medium text-gray-500 dark:text-gray-400">Progress</h4>
                                </div>
                                ...
                                @if($import->total_rows > 0)
                                    <div class="w-full h-2 mt-2 bg-gray-200 rounded-full dark:bg-gray-700 overflow-hidden">
                                        <div class="h-2 bg-primary-600 rounded-full transition-all duration-300" style="width: {{ ($import->successful_rows / $import->total_rows) * 100 }}%"></div>
                                    </div>
                ...
                @if($import->failed_rows > 0 && $import->errors)
                    <div class="bg-red-50 dark:bg-red-900/10 border border-red-200 dark:border-red-800 rounded-lg overflow-hidden shadow-sm">
                        <div class="p-4 sm:p-6">
                            <div class="flex items-center gap-2 mb-4">
                                <svg class="w-5 h-5 text-red-600 dark:text-red-400 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"/>
                                </svg>
                                <h3 class="text-base sm:text-lg font-semibold text-red-900 dark:text-red-400">
                            ...
                            <div class="hidden md:block overflow-x-auto">
                                <table class="min-w-full divide-y divide-red-200 dark:divide-red-800">
                                    <thead class="bg-red-100 dark:bg-red-900/20">
                                        <tr>
                                            <th class="px-4 py-3 text-xs font-semibold text-left text-red-700 uppercase tracking-wider dark:text-red-300">Row</th>
                                            <th class="px-4 py-3 text-xs font-semibold text-left text-red-700 uppercase tracking-wider dark:text-red-300">Error Message</th>
                                        </tr>
                                    </thead>
                    ...
                    <div class="bg-white dark:bg-[#171717] border border-gray-200 dark:border-gray-800 rounded-lg shadow-sm overflow-hidden">
                        <div class="p-4 sm:p-6">
                            <div class="flex items-center gap-2 mb-4">
                                ...
                                <h3 class="text-base sm:text-lg font-semibold text-gray-900 dark:text-gray-100">Data Preview</h3>
                            </div>
                                            ...
                                            <tr>
                                                @if(!empty($previewData[0]))
                                                    @foreach($previewData[0] as $column)
                                                        <th class="px-3 sm:px-4 py-3 text-xs font-semibold text-left text-gray-600 uppercase tracking-wider dark:text-gray-400 whitespace-nowrap">{{ $column }}</th>
                                                    @endforeach
                                                @endif
                                            </tr>
                                        </thead>
                                            ...
                                            @forelse(array_slice($previewData ?? [], 1) as $row)
                                                <tr class="hover:bg-gray-50 dark:hover:bg-[#0a0a0a] transition-colors">
                                                    @foreach($row as $value)
                                                        <td class="px-3 sm:px-4 py-3 text-gray-900 dark:text-gray-100 whitespace-nowrap">{{ $value }}</td>
                                                    @endforeach
                                                </tr>
                                            @empty
                                                <tr>
                                                    <td colspan="100" class="px-4 py-8 text-center text-sm text-gray-500 dark:text-gray-400">
                    ...
                    <div class="bg-white dark:bg-[#171717] border border-gray-200 dark:border-gray-800 rounded-lg shadow-sm overflow-hidden">
                        <div class="p-4 sm:p-6">
                            <div class="flex items-center gap-2 mb-4">
                                ...
                                <h3 class="text-base sm:text-lg font-semibold text-gray-900 dark:text-gray-100">Recent Transactions</h3>
                            </div>
                                    ...
                                    <div class="bg-gray-50 dark:bg-[#0a0a0a] border border-gray-200 dark:border-gray-800 rounded-lg p-3">
                                        <div class="flex items-start justify-between mb-2">
                                            <div class="flex-1 min-w-0">
                                                <p class="text-sm font-semibold text-gray-900 dark:text-gray-100 truncate">{{ $transaction->transaction_code }}</p>
                                                <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">{{ $transaction->timestamp?->format('M d, Y') ?? 'N/A' }}</p>
                                            </div>
                                            <span class="text-sm font-semibold text-gray-900 dark:text-gray-100 flex-shrink-0">₱{{ number_format($transaction->total, 2) }}</span>
                                        </div>
                            ...
                            <div class="hidden md:block overflow-x-auto -mx-4 sm:-mx-6">
                                <div class="inline-block min-w-full align-middle px-4 sm:px-6">
                                    <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-800">
                                        <thead class="bg-gray-50 dark:bg-[#0a0a0a]">
                                            <tr>
                                                <th class="px-4 py-3 text-xs font-semibold text-left text-gray-600 uppercase tracking-wider dark:text-gray-400">Code</th>
                                                <th class="px-4 py-3 text-xs font-semibold text-left text-gray-600 uppercase tracking-wider dark:text-gray-400">Date</th>
                                                <th class="px-4 py-3 text-xs font-semibold text-left text-gray-600 uppercase tracking-wider dark:text-gray-400">Customer</th>
                                                <th class="px-4 py-3 text-xs font-semibold text-left text-gray-600 uppercase tracking-wider dark:text-gray-400">Items</th>
                                                <th class="px-4 py-3 text-xs font-semibold text-right text-gray-600 uppercase tracking-wider dark:text-gray-400">Total</th>
                                            </tr>
                                        </thead>
                                        <tbody class="bg-white divide-y divide-gray-200 dark:bg-[#171717] dark:divide-gray-800">
